...
class Image {
     ...
     */
    public function __construct(int t = self::IMAGE_GD, string imgname = null) {
    	string class_name;
    	if t == self::IMAGE_GD {
    		let class_name = "Gd";
    	} elseif t == self::IMAGE_IMAGICK {
    		let class_name = "Imagick";
    	} else {
    		throw new \Exception("不支持的图片处理库类型");
    	}
        /* 引入处理库，实例化图片处理对象 */
        let class_name = "winer\\Image\\Driver\\" . class_name;
        let this->img = new {class_name}(imgname);
    }
     ...
     */
    public function open(string imgname) {
     ...
     * @return Object 当前图片处理库对象
     */
    public function save(string imgname, t = null, int quality = 80, boolean interlace = true) {
        this->img->save(imgname, t, quality, interlace);
        return this;
    }
     ...
     * @return Object         当前图片处理库对象
     */
    public function crop(int w, int h, int x = 0, int y = 0, int width = null, int height = null){
     ...
     * @return Object         当前图片处理库对象
     */
    public function thumb(int width, int height, int t = self::IMAGE_THUMB_SCALE) {
        this->img->thumb(width, height, t);
        return this;
    }
     ...
     * @return Object          当前图片处理库对象
     */
    public function water(string source, int locate = self::IMAGE_WATER_SOUTHEAST, int alpha = 80) {
     ...
     * @return Object          当前图片处理库对象
     */
    public function text(text, font, size, string color = "#00000000", 
        int locate = self::IMAGE_WATER_SOUTHEAST, int offset = 0, int angle = 0) {
